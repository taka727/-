...
#LSTMモデルの学習
def LSTMtemple_Training(model,X_train,Y_train,batch,n_epoch,hidden,place,number):
...
def csv_save(predicted_data,EPOCH,HIDDEN,PLACE,MODEL):
  with open('/Users/konishitakashidai/Desktop/卒研/卒研プログラム/ENEM/ENEM'+PLACE+'/predictENEM'+str(EPOCH)+str(HIDDEN)+PLACE+"predictENSEMBLE"+MODEL+".csv", "w", encoding="Shift_jis") as f:
...
def study(PLACE,HIDDEN,EPOCH):
  ...
  temple_predicted1= LSTMtemple_Training(LSTM_temple_model1,temple_Xtrain,temple_Ytrain,batch_size,epoch,n_hidden,PLACE,1)
  ...
  temple_predicted2= LSTMtemple_Training(LSTM_temple_model2,temple1,temple_Ytrain,batch_size,epoch,n_hidden,PLACE,2)
  #訓練用のデータを追加する
...
def predict(PLACE,EPOCH,HIDDEN):
  FEATURE_VALUE = ['average_temperature']
  LENGTH_PER_UNIT = 719
  DIMENSION = len(FEATURE_VALUE)
  #予測結果保存用フォルダ作成
  ...
  os.mkdir(new_dir_path)
  new_dir_path = '/Users/konishitakashidai/Desktop/卒研/卒研プログラム/ENEM/ENEM'+PLACE+'/predictENEM'+str(EPOCH)+str(HIDDEN)+PLACE
  os.mkdir(new_dir_path)
  #予測期間
  ...
  LSTM_model1,LSTM_model2,ENEM_model,RNN_model = model_load(EPOCH,HIDDEN,PLACE,1,2) 
  ...
  for l in range(0,4):
    ...
    csv_save(LSTM_predict,EPOCH,HIDDEN,PLACE,"LSTM1")
    csv_save(LSTM_predict2,EPOCH,HIDDEN,PLACE,"LSTM2")
    csv_save(ensemble_predict,EPOCH,HIDDEN,PLACE,"ENEM")
    csv_save(RNN_predict,EPOCH,HIDDEN,PLACE,"RNN")
    predicted_ave =[]
    print("結果の平均値の算出")
    for m in range (0,len(ensemble_predict)):
      temple_value =[]
      sum = ensemble_predict[m][0]+RNN_predict[m][0]
      ave = sum/2
      aver = ave.tolist()
      temple_value.append(aver)
      predicted_ave.append(temple_value)

    csv_save(predicted_ave,EPOCH,HIDDEN,PLACE,"AVE")
    Y =[]
    predicted_data = []
    ...
    '/predictENEM'+str(EPOCH)+str(HIDDEN)+PLACE+
    "/predictENEM"+str(EPOCH)+str(HIDDEN)+TEST_periodstart[l]+'-'+TEST_periodend[l]+PLACE+".csv")
...
def main():
  ...
  for l in range(0,5):
    for i in range(0,5):
      predict(place[i],hidenn[l],epoch[l])
